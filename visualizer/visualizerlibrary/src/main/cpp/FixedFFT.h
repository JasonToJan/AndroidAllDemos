...
// Modified by me, Carlos Rafael Gimenes das Neves
// Originals:
// https://android.googlesource.com/platform/system/media/+/master/audio_utils/fixedfft.cpp
// https://android.googlesource.com/platform/frameworks/av/+/master/media/libmedia/Visualizer.cpp
...
#define v workspace
	...
	for (p = 1; p < n; p <<= 1) {
		--scale;
		for (i = 0; i < n; i += p << 1) {
			const int32_t x = half(v[i]);
			const int32_t y = half(v[i + p]);
			v[i] = x + y;
			v[i + p] = x - y;
		}
		for (r = 1; r < p; ++r) {
	...
	for (i = 1; i < n >> 1; ++i) {
		int32_t x = half(v[i]);
		const int32_t z = half(v[n - i]);
		int32_t y = z - (x ^ 0xFFFF);
		x = half(x + (z ^ 0xFFFF));
		y = mult(y, ((int32_t) twiddle[i]));
		v[i] = x - y;
		v[n - i] = (x + y) ^ 0xFFFF;
	}
#undef n
