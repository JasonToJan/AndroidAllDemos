...
public class ThingDetailFragment extends TransitionHelper.BaseFragment {
    ...
    @BindView(R2.id.detail_title) TextView titleTextView;
    @BindView(R2.id.detail_body) TextView detailBodyTextView;
    @BindView(R2.id.overscroll_view) OverScrollView scrollView;
        ...
        scrollView.setOverScrollListener(new OverScrollView.OverScrollListener() {
            ...
            public boolean onOverScroll(int yDistance, boolean isReleased) {
                if (Math.abs(yDistance) > translationThreshold) { //passed threshold
                    ...
                    TransitionMaterialBaseActivity.of(getActivity()).animateHomeIcon(MaterialMenuDrawable.IconState.ARROW);
                }
                return false;
            }
        });
    ...
    public void onBeforeViewShows(View contentView) {
        ...
        TransitionMaterialBaseActivity.of(getActivity()).fab.setTranslationY(400);
    ...
    public void onBeforeEnter(View contentView) {
        TransitionMaterialBaseActivity.of(getActivity()).fragmentBackround.animate().scaleX(.92f).scaleY(.92f).alpha(.6f).setDuration(Navigator.ANIM_DURATION).setInterpolator(new AccelerateInterpolator()).start();
        TransitionMaterialBaseActivity.of(getActivity()).setHomeIcon(MaterialMenuDrawable.IconState.BURGER);
        TransitionMaterialBaseActivity.of(getActivity()).animateHomeIcon(MaterialMenuDrawable.IconState.ARROW);
    }
    ...
    public boolean onBeforeBack() {
        ...
        TransitionMaterialBaseActivity.of(getActivity()).fragmentBackround.animate().scaleX(1).scaleY(1).alpha(1).translationY(0).setDuration(Navigator.ANIM_DURATION/4).setInterpolator(new DecelerateInterpolator()).start();
